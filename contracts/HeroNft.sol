...
// File: contracts/10_HeroNft721.sol
...
contract Hero721 is ERC721 {
    ...
    function setCrypto(address crypto_) public { 
        require(_operators[msg.sender], "only for operator");
        _crypto = IERC20(crypto_);
    }
    ...
    function _tokenTransferFrom(address from_, address to_, uint256 amount_) internal returns (uint256) {
        if(amount_ == 0) return 0;
        // use native
        if(_crypto == IERC20(address(0))) {
            require( msg.value >= amount_, "not enough");
            return 1;
        }
        // use token    
        require( crypto().allowance(from_, to_) >= _price, "need approved");
        crypto().transferFrom(from_, to_, amount_);
        return 2;
        
    }
    
    function _tokenTransfer(address to_, uint256 amount_) internal returns (uint256) {
        if(amount_ == 0) return 0;
        // use native
        if(_crypto == IERC20(address(0))) {
        ...
        // use token
        require( crypto().balanceOf(address(this)) >= amount_, "not enough");
        crypto().transfer(to_, amount_);
        return 2;
    }
    
    function setPrice(uint256 num_, uint256 val_, uint256 percent_) public { 
        require(_operators[msg.sender], "only for operator");
        _price      = val_;
        _tokenNum   = num_;
        _percent    = percent_;
    ...
    function getTax() public view returns (uint256) { 
        ...
        // use native
        if(_crypto == IERC20(address(0))) {
            return address(this).balance - _commission;
        }
        // use token
    ...
    function mintProject(string memory tokenURI_, address[] memory tos_, uint256 index_, string memory ext_, uint256 amount_) external payable returns (uint256) { 
        require(_operators[msg.sender], "only for operator");
        require( tos_.length <= _tokenNum, "invalid token number");
        require( amount_  == _price * tos_.length,  "Amount sent is not correct" );
        _tokenTransferFrom(msg.sender, address(this), amount_);
        ...
        for(uint256 i = 0; i < tos_.length; i++) {
            ...
            _URLs[newItemId]    = string(abi.encodePacked(tokenURI_,name,ext_));
        }
    ...
    function withdraw() external {
        require(_creator == msg.sender, "only for creator");
    ...
    function withdrawTax() external {
        require(_owner == msg.sender, "only for owner");
        uint256 amount = getTax();
        _tokenTransfer(msg.sender, amount);
    }
    // TODO not use on mainnet
